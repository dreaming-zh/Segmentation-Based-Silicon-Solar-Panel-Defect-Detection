...
def show_result(img_id, input_shape):
    ...
    fig  = result[0][0,...,1]
    fig1 = result[0][0,...]
    fig1 = np.reshape(fig1,(1,608*608,2))
    fig1 = np.argmax(fig1,axis=2).astype(np.int8)
    fig1 = np.reshape(fig1,(608,608))
...
if __name__ == '__main__':
    ...
    batch_size = 8
    epoch = 30
    logdir = '/root/zsh/logs/' + datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    model_loadname = '20221215-193338_v1'
    # model_loadname = ''
    model_savename  = datetime.now().strftime("%Y%m%d-%H%M%S") + '_v2'
    ...
    if os.listdir(model_savepath):
        try: 
            model.load_weights(model_savepath + model_loadname + '_acc_weights.h5')
            print('Model weights loaded!')
        except:
            print('Model weights load filed!')
    
    save_best_1 = ModelCheckpoint(model_savepath + model_savename + '_loss_weights.h5', monitor='val_seg_output_loss', 
                                  verbose=1, save_best_only=True, save_weights_only=True, mode='auto')
    save_best_2 = ModelCheckpoint(model_savepath + model_savename + '_acc_weights.h5', monitor='val_class_output_acc', 
                                  verbose=1, save_best_only=True, save_weights_only=True, mode='auto')
    early_stop = EarlyStopping(monitor='val_loss', min_delta=0, patience=25, verbose=0, mode='min')
    h = model.fit_generator(generator=batch_generater(train_set_path,train_name_list,batch_size,input_shape,n_label), 
                            ...
                            epochs=epoch, verbose=1, callbacks=[save_best_1, save_best_2, early_stop, tensorboard_callback], 
    ...
    plt.title('seg_output_loss')
    plt.ylabel('loss')
    plt.xlabel('epoch')
    plt.legend(['train', 'validation'])
    plt.savefig('train_val_loss_up_v2.png',dpi=600)
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'validation'])
    plt.savefig('train_val_acc_up_v2.png',dpi=600)
    
    show_result(np.random.randint(0,600),input_shape)